...
class Net(nn.Module):
    ...
    def forward(self, exo, ego, aff_label, egocentric_image_path, epoch):
        num_exo = exo.shape[1]
        ...
        with torch.no_grad():
            ...
            ego_desc0 = ego_key.permute(0, 2, 3, 1).flatten(-2, -1).detach()
            exo_desc = exo_key.permute(0, 2, 3, 1).flatten(-2, -1).detach()

        ego_proj = ego_desc0[:, 1:] + self.aff_proj(ego_desc0[:, 1:])
        exo_proj = exo_desc[:, 1:] + self.aff_proj(exo_desc[:, 1:])
        ego_desc = self._reshape_transform(ego_desc0[:, 1:, :], self.patch, self.stride)
        ...
        exo_funcparts_mask = []
        exo_funcparts_sum = []
        valid_values_tensor = torch.zeros_like(gt_aff_cam).cuda()
        ...
        for b_ in range(b):
            ...
            for n in range(num_exo):
                ...
                # 获取掩码
                mask = tmp_cam > self.aff_cam_thd

                # 恢复到 [384, 28, 28]
                valid_flattened = tmp_cam * mask.float()
                valid_values_tensor[b_, n] = valid_flattened.reshape(28, 28)
                ...
                exo_funcparts_sum.append(exo_funcparts_tensor)
            exo_funcparts_sum_t = torch.cat(exo_funcparts_sum, dim=0)  # 3,384
            ...
            for n_ in range(num_exo):
                ...
                exo_funcparts_sim_maps[b_, n_] = torch.mm(exo_funcparts_sum_t[n_].unsqueeze(0), exo_desc_re_flat0)
            ...
            exo_funcparts_sum_mean =exo_funcparts_sum_t.mean(dim=0)
            part_func_proto[b_] = exo_funcparts_sum_mean  # exo_func_part_proto
        ...
        aff_func_ego = self.gap(ego_func).view(b, self.aff_classes)
        # --- concentration loss --- #
        ...
        masks = {'exo_aff': (valid_values_tensor, exo_aff_desc_sum, part_func_proto), 'ego_sam': ego_sam,
                 'pred': (sim_func_maps, func_ego_cam, part_score, gt_ego_cam)}
        logits = {'aff_exo': aff_exo_logits, 'aff_ego': aff_logits_ego, 'aff_func_ego': aff_func_ego}
    ...
    def handaround_feature_extact(self, exo):
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                is_parallel = True
                for i in range(1, 4):
                    norm_i = np.linalg.norm(finger_vectors[i])
                    norm_i_plus_1 = np.linalg.norm(finger_vectors[i + 1])
                    angle_cosine = np.dot(finger_vectors[i], finger_vectors[i + 1]) / (norm_i * norm_i_plus_1)
                    # angle_cosine = np.dot(finger_vectors[i], finger_vectors[i + 1])  # 计算夹角余弦值
                    if angle_cosine < 0.966:  #0.966，0.996, 0.866，0.940
                        is_parallel = False
                        break
