...
def overlay_mask_yf(img: Image.Image, mask: Image.Image, colormap: str = "jet", alpha: float = 0.7) -> Image.Image:
    if not isinstance(img, Image.Image) or not isinstance(mask, Image.Image):
        raise TypeError("img and mask arguments need to be PIL.Image")
    ...
    # --------------------locate the most red point ------------------#
    # Extract the red channel
    red_channel = overlay[:, :, 0]

    # Find the index of the brightest red point
    brightest_red_index = np.unravel_index(np.argmax(red_channel), red_channel.shape)

    # The coordinates of the brightest red point
    center_x, center_y = brightest_red_index[::-1]  # reverse to get (x, y)

    return overlayed_img, center_x, center_y
    # --------------------locate the most red point ------------------#

    # --------------------locate the most highlight point ------------------#
    # grayscale = np.dot(overlay[..., :3], [0.2989, 0.5870, 0.1140])
    #
    # # Find the index of the brightest point
    # brightest_index = np.unravel_index(np.argmax(grayscale), grayscale.shape)
    #
    # # The coordinates of the brightest point
    # center_x, center_y = brightest_index[::-1]  # reverse to get (x, y)
    #
    # return overlayed_img, center_x, center_y
    # --------------------locate the most highlight point ------------------#